...
namespace ecn
{
    ...
    void reconstructPath(PtrMap<T> &come_from, T *best, int dist)
    {
        ...
        std::cout << "solved in " << summary.size() - 1 << " steps, distance is " << dist << std::endl;
    }
    ...
    void Astar(T start, T goal)
    {
        ...
        class priority_access : public std::priority_queue<NodeWithCost, std::vector<NodeWithCost>, Compare>
        {
        public:
        ...
        while (!queue.empty())
        {
            ...
            for (auto &child : children)
            {
                ...
                if (std::find_if(closedSet.rbegin(), closedSet.rend(),
                                 [&child_ptr](T *elem)
                                 { return elem->is(*child_ptr); }) == closedSet.rend())
                {
